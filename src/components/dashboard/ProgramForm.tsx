...
import type { Program, BookingType, EventCategory } from "@/types/database";
...
): Promise<{ title?: string; date?: string; time?: string; description?: string }> {
...
// Generate all dates (YYYY-MM-DD) for a recurrence between start and end
...
export function ProgramForm({ centerId, program, onSuccess, onCancel }: Props) {
  const [form, setForm] = useState({
    title: program?.title ?? "",
    description: program?.description ?? "",
    date: program?.date ?? "",
    time: program?.time ?? "",
    booking_type: (program?.booking_type ?? "none") as BookingType,
    booking_url: program?.booking_url ?? "",
    youtube_stream_url: program?.youtube_stream_url ?? "",
    category: (program?.category ?? "other") as EventCategory,
  });
  ...
  async function handleExtract() {
    ...
    try {
      ...
      if (aiImage) {
        ...
        result = await extractFromAI(undefined, base64, aiImage.type);
        // Also use image as poster
        setPosterFile(aiImage);
      } else {
        result = await extractFromAI(aiText);
      }
      setForm((f) => ({
        ...f,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let poster_image_url = program?.poster_image_url ?? null;
    ...
    // Editing an existing program — single update, no recurrence
    if (program) {
      const res = await fetch(`/api/dashboard/events/${program.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...base, date: form.date || null }),
      });
      if (!res.ok) setError("Failed to update event");
      else onSuccess();
      setLoading(false);
      return;
    }

    // Creating — generate dates for recurrence
    ...
    if (!res.ok) setError("Failed to create event");
    else onSuccess();
    setLoading(false);
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {/* AI extraction — only when creating */}
      {!program && (
      ...
      {/* Recurrence — only shown when creating */}
      {!program && (
